...
type Props = {
  ...
  /** 폴더명 수정 '저장' 클릭 */
  onSubmitUpdateGroupName?: (groupId: string, name: string) => Promise<void> | void;
  /** 폴더 삭제 */
...
const PortfolioAssetItem = memo(({ asset }: PortfolioAssetItemProps) => {
  const realtimeStockId = toNumericStockId(asset.stockId) ?? -1;
...
const SimulationPortfolioTab: React.FC<Props> = ({
  ...
  const [portfolioAssets, setPortfolioAssets] = useState<Record<string, PortfolioAsset[]>>({});
  ...
  useEffect(() => {
    ...
    const loadAssetCounts = async () => {
      const assetCounts: Record<string, PortfolioAsset[]> = {};
  ...
  const submitDelete = async (groupId: string) => {
    try {
  ...
  useEffect(() => {
    ...
    expandedGroupIds.forEach((groupId) => {
      ...
      assets.forEach((asset) => {
        const realtimeStockId = toNumericStockId(asset.stockId);
        if (realtimeStockId != null) {
          stockIds.push(realtimeStockId);
        }
      });
    });
